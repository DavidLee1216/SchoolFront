...
function TeacherProfilePage(props) {
  ...
  const [values, setValues] = useState({
    ...
    speak_lang: props.teacherInfo.speak_lang,
  ...
  const addSpeakingLanguage = () => {
    if (values.speak_lang.length >= 3) {
      notify.show("Can't add speaking language more than 3.", "error", 2000);
      return;
    }
    setValues({
      ...values,
      speak_lang: [...values.speak_lang, { lang: "", level: "" }],
    });
  };

  const deleteSpeakingLanguage = (idx) => {
    setValues({
      ...values,
      speak_lang: [
        ...values.speak_lang.slice(0, idx),
        ...values.speak_lang.slice(idx + 1),
      ],
    });
  };
                              ...
                              >
                                  ...
                                  <Autocomplete
                                    ...
                                    onChange={(event, value) => {
                                      setValues({
                                        ...values,
                                        speak_lang: [
                                          ...values.speak_lang.slice(
                                            0,
                                            index + 1
                                          ),
                                          {
                                            ...values.speak_lang[index + 1],
                                            lang:
                                              value != null ? value.lang : "",
                                          },
                                          ...values.speak_lang.slice(index + 2),
                                        ],
                                      });
                                    }}
                                  />
                                ...
                                <Grid item xs={12} sm={2}>
                                  <Button
                                    ...
                                    fullWidth
                                    onClick={() =>
                                      deleteSpeakingLanguage(index + 1)
                                    }
                                  >
                                    Delete
                                  </Button>
...
const getTeacherInfo = () => {
  ...
  var teacherInfo = {
    ...
    speak_lang: [{ lang: "Russian", level: "intermediate" }],
